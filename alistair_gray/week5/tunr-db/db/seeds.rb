...
s2 = Song.create! title: 'Have a Safe Tri, Dear'
...
art1.songs << s1 # achy breaky heart - billy ray cyrus
...
art3.songs << s3 << s4 # burn the witch and identikit from Radiohead

puts "Testing artist -< songs associations"
puts "The song #{Song.first.title} by the artist #{Song.first.artist.name}"
puts "The artist #{Artist.last.name} has the songs #{Artist.last.songs.pluck(:title).join(', ')}"

# Create associations from albums to their songs
alb1.songs << s1
alb2.songs << s2
alb3.songs << s3 << s4

puts "Testing album -< songs associations"
puts "The song #{Song.first.title} has the album #{Song.first.album.title}"
...
# Create associations from genres to songs
